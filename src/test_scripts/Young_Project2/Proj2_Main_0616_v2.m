...
kp_depth = 100.0; ki_depth = 150; kd_depth =  50.0; % Depth
kp_heading = 1.5; ki_heading = 0.5; kd_heading = 0.75; % Heading
stabilityThrust = 500; 
% watchCircleRadius = 0.5;
...
%% Define waypoint objects using wpt = PoseDefinition(rpm,wcr,x,y,z,x_o,y_o,z_o,w)
wptStart = PoseDefinition(20, 1, -3, -3, -48, 0, 0, 0, 1); % vehicle always starts here, 
wptInit  = PoseDefinition(20, 1,-6, -6, -45, 0, 0, 0, 1); % always goes here first
wptHome1 = PoseDefinition(10, 0.25, 0, 0, -47.3, -0.005, 0.001, 0.770, -0.638); % ends here
wptHome2 = PoseDefinition(10, 0.5, 0, 0, -50, -0.005, 0.001, 0.770, -0.638);   % then lowers to here
...
tag_00   = PoseDefinition(20, 2, 0.0, 5.5, -51.88, -0.005, 0.004, 0.745, -0.667);
tag_01   = PoseDefinition(20, 2, 0.0, 5.5, -55.786, -0.005, 0.001, 0.770, -0.638);
tag_02   = PoseDefinition(20, 2, 0.0, 5.5, -56.969, -0.006, 0.001, 0.745, -0.667);
tag_03   = PoseDefinition(20, 2, 0.0, -5.5, -51.989, -0.004, 0.0, -0.669, -0.743);
tag_04   = PoseDefinition(20, 2, 0.0, -5.5, -55.747, 0.0, 0.0, -0.680, -0.733);
tag_05   = PoseDefinition(20, 2, 0.0, -5.5, -57.023, 0.0, 0.0, -0.679, -0.734);
tag_06   = PoseDefinition(20, 2, 5.5, 0.0, -51.756, 0.0, 0.0, -1.0, -0.016);
tag_07   = PoseDefinition(20, 2, 5.5, 0.0, -52.127, 0.0, 0.0, -1.0, -0.16);
tag_08   = PoseDefinition(20, 2, 5.5, 0.0, -52.606, 0.0, 0.0, -1.0, -0.016);
tag_09   = PoseDefinition(20, 2, 5.5, 0.0, -52.920, 0.0, 0.0, -1.0, -0.016);
tag_10   = PoseDefinition(20, 2, 5.5, 0.0, -53.250, 0.0, 0.0, -1.0, -0.017);
tag_11   = PoseDefinition(20, 2, 5.5, 0.0, -54.338, 0.0, 0.0, -1.0, -0.013);
...
% finalRun = [wptInit, tag_00, tag_01, wptHome1, wptHome2];
...
while (ind <= length(navArray)) % global navigation loop
    ...
    watchCircleRadius = navArray(ind).wcr;
    navPoints = findNavigationPath(sv, start, goal, planner, ind);
    [m_nav, n_nav] = size(navPoints);
    
    %% Navigation from one point to next
    jj = 1;
    while (jj < m_nav) % single path navigation loop
        ...
        if abs(depthError) >= maxDepthError
            ...
            if abs(headingError) >= maxHeadingError 
                ...
                rpmAdjustment = navArray(ind).rpm;
                %% Update Message Data
                [vert_port_thruster_msg, vert_stbd_thruster_msg, ...
                    bow_port_thruster_msg, bow_stbd_thruster_msg, ...
                    aft_port_thruster_msg, aft_stbd_thruster_msg] = ...
...
%%%                if (goal(1) == nextTag.x) && (goal(2) == nextTag.y)
%%%                   nextGoal = [];
...
%%%                else
                    ...
                    if isFinalRun == false
                        ...
                        navArray(ind+1) ...
                            = PoseDefinition(20, 2, nextTag.x, nextTag.y, ...
                                             nextTag.z, nextTag.h);
                    end
                    break
%%%                end
                % break out after specified number of check and return to
                % home position
                % if (checkCount >= maxTagChecks)% || (nextGoal == 11)
                %     ind = length(navArray) - 3;
                %     sprintf('No valid "new goal". Returning to home');
                %     break
                % end
